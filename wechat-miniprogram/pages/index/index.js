...
Page({
  ...
  onCopyShare() {
    ...
    if (d.needVerify || d.carPassword === '点击验证密码') {
      this.openVerifyModal();
      return;
    }
    ...
    if (d.carPassword && d.carPassword !== '--') {
      hasPassword = true;
      lines.push(`1. 系统口令：${d.carPassword}`);
      if (d.carInstructions) lines.push(`  使用说明：${d.carInstructions}`);
    }
    if (d.settingPassword && d.settingPassword !== '--') {
      hasPassword = true;
      lines.push(`2. 设置口令：${d.settingPassword}`);
      if (d.settingInstructions) lines.push(`  使用说明：${d.settingInstructions}`);
    }
